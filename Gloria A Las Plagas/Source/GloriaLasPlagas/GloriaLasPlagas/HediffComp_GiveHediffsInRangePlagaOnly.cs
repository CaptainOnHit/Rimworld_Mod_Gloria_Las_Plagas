...
{
    public class HediffComp_GiveHediffsInRangePlagaOnly : HediffComp_GiveHediffsInRange
    {
        private Mote mote;

        public HediffCompProperties_GiveHediffsInRangePlagaOnly Props => (HediffCompProperties_GiveHediffsInRangePlagaOnly)props;
        ...
        public override void CompPostTick(ref float severityAdjustment)
        {
            ...
            {
                //only appanded one extra check so only pawns with the plaga parasite recieve the buff
                if (!item.RaceProps.Humanlike || item.Dead || item.health == null || item == parent.pawn || !(item.Position.DistanceTo(parent.pawn.Position) <= Props.range) || !Props.targetingParameters.CanTarget(item) || !DefNameHelper.ValidatePlagaXenotype(item, false))
                {
                    continue;
                }
